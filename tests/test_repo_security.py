...
# Generic secret patterns that MUST NOT appear in any tracked repository file
FORBIDDEN_REGEXES = [
    (re.compile(r"eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+"), "Raw JWT literal"),
    (re.compile(r"prism_session_token_pattern"), "Real session/access token cookie literal"),
    (re.compile(r"gAAAAA[A-Za-z0-9_-]{30,}"), "Real sandbox token literal"),
]
...
def test_repo_has_no_leaked_secrets_or_real_identifiers():
    """Scan all tracked source, doc, and test files for leaked secrets and historical real identifiers."""
    failures = []
